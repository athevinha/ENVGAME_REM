...
class create_model:
  def __init__(self, data_dir, img_height,img_width,model_training,zip_files,epoch = 5, name_model = "user",batch_size = 64):
    ...
    self.data_augmentation = tf.keras.Sequential([
        ...
        tf.keras.layers.CenterCrop(self.img_height, self.img_width ),
        tf.keras.layers.RandomZoom(.5, .2)
    ])
    self.train_ds = self.train_ds.map(lambda x, y: (self.data_augmentation(x), tf.one_hot(y, depth=self.num_classes)))
    self.val_ds = self.val_ds.map(lambda x, y: (self.data_augmentation(x), tf.one_hot(y, depth=self.num_classes)))
  ...
  def mobileNet(self,type_model):
    ...
    if(H.history['accuracy'][-1] <= 0.85):
      ...
      model.compile(SGD(0.001), 'categorical_crossentropy', ['accuracy'])
